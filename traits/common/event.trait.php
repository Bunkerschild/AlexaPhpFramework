...
trait event
{
    ...
    {
        $this->header = new \AlexaPhpFramework\common\header($event->header);
        
        if (isset($event->endpoint))
            $this->endpoint = new \AlexaPhpFramework\common\endpoint($event->endpoint);
        else
            unset($this->endpoint);
            
        $this->payload = new \AlexaPhpFramework\common\payload(new \stdClass);
    }
}
